...
from other import Bot, logger, Queue, save_key_value, get_data_from_key, delete_by_key
...
async def get_timer(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    Q.insert_data(url=url, timer=timer)
    await message.answer('Сообщение добавлено в очередь на закреп')
    # try:
    #     # logger.info('Как эта хуйня работает?')
    ...
    #     # )
    #     # logger.info(f'{url=}\n{timer=}')
    #     # await Queue.insert_data(url=url, timer=timer)
    # except Exception as e:
    #     await message.answer(f'Произошла ошибка\n{e}')
    #     # await message.answer('Произошла ошибка, бот не состоит в этом чате или не является'
    #     #                      'администратором')
