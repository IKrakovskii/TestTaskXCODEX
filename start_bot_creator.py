...
async def start(message: Message, state: FSMContext):
    ...
    if is_super_admin(message):
        r = await message.answer('/create_bot - создать бота'
                                 ...
                                 '\n/get_admins - получить спиооа админовов'
...
@form_router.message(FSM.get_admins)
@logger.catch
async def get_admins(message: Message, state: FSMContext):
    ...
    os.system(f'docker run -d --name {get_data_from_key(f"{message.from_user.id}_bot_name")}_pinner_bot -e token={get_data_from_key(f"{message.from_user.id}_bot_token")} -e ids={get_data_from_key(f"{message.from_user.id}_admins")} pinner_bot')
